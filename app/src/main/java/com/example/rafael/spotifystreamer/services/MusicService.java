    ...
    private void updateMediaSessionMetaData(Bitmap image) throws IOException {
        if (android.os.Build.VERSION.SDK_INT >= 21) {
            mMetadatav21 = new MediaMetadata.Builder()
                    ...
                    .putString(MediaMetadata.METADATA_KEY_ALBUM, myTrackList.get(songPosition).getTrackAlbum())
                    .putString(MediaMetadata.METADATA_KEY_TITLE, myTrackList.get(songPosition).getTrackName())
                    .putLong(MediaMetadata.METADATA_KEY_DURATION, getDur())
                    .putLong(MediaMetadata.METADATA_KEY_TRACK_NUMBER, songPosition)
                    .putLong(MediaMetadata.METADATA_KEY_NUM_TRACKS, myTrackList.size())
